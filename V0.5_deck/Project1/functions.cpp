...
void isvestis_file(deque <Studentas>& temp)
{
    ...
    {
        sprintf_s(eilute, "%-20s %-20s %-16.2lf %-17.2lf\n", x.vardas.c_str(), x.pavarde.c_str(), x.rez, x.med);
        output += eilute;
    }
    out_f << output;
    out_f.close();
...
void read_file(deque <Studentas>& sar)
{
    Studentas st;
    string line;
    int nd, egz, lines;
    ...
    cout << "Failo is " << lines << " eiluciu nuskaitymas ir sudejimas i decka uztruko : " << diff.count() << " s\n";
}
