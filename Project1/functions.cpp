...
void isvestis_2files(vector <Studentas> temp)
{
    ...
    std::ofstream out_f("Silpnuciai.txt");
    out_f << left << setw(21) << "Vardas" << left << setw(21) << "Pavarde" << "Galutinis vid. " << "/ Galutinis med.\n";
    out_f << "-------------------------------------------------------------------------\n";
    while (temp.at(i).rez<5)
    {
        sprintf_s(eilute, "%-20s %-20s %-16.2lf %-17.2lf\n", temp.at(i).vardas.c_str(), temp.at(i).pavarde.c_str(), temp.at(i).rez, temp.at(i).med);
        output += eilute;
        i++;
    ...
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end - start;
    cout <<"isvestis i faila Silpnuciai.txt uztruko : "<< diff.count()<<" s\n";
    output.clear();
    auto start2 = std::chrono::high_resolution_clock::now();
    std::ofstream out_f2("Einsteinai.txt");
    ...
    cout << "isvestis i faila Einsteinai.txt uztruko : " << diff2.count() << " s\n";
    temp.clear();
}
...
void isvestis_2files(vector <Studentas> temp, string a, string b)
{
    ...
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = end - start;
    cout << "isvestis i faila "<<a<<" uztruko : " << diff.count() << " s\n";
    output.clear();
    auto start2 = std::chrono::high_resolution_clock::now();
    ...
    auto end2 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff2 = end2 - start2;
    cout << "isvestis i faila " <<b<<" uztruko : " << diff2.count() << " s\n";
    temp.clear();
}