        ...
        do
        {
            ...
            if (nd) //tikrinama, ar ivestas skaicius
            {
                if (nd >= 0 && nd <= 10) //tikrinama, ar pazymys yra nuo 0 iki 10
                {
                    ...
                    temp.nd.push_back(nd); //pazymys pridedamas i list konteineri
    ...
    else //jei vartotojas pasirenka generuoti skaicius (choice==2)
    {
        ...
        do
        {
            ...
            if (skaitl > 0)
            {
                ...
                {
                    nd = dist(mt);
                    temp.nd.push_back(nd); //balas pridedamas i studento list konteineri
                    temp.sum += nd;
                }
...
void read_file(list <Studentas>& sar, string a)
{
    ...
    cout << "Failo is " << lines << " eiluciu nuskaitymas ir sudejimas i lista uztruko : " << diff.count() << " s\n";
}
...
void isvestis_2files(list <Studentas>& temp, string a, string b)
{
    ...
    std::list<Studentas>::iterator it = temp.begin();
        while(it->rez<5)
    {
            bad.push_back(*it);
            temp.pop_front();
    ...
    for (const auto& x : temp)
    {
        output2 << left << setw(20) << x.vardas << left << setw(20) << x.pavarde << std::fixed << std::setprecision(2) << setw(17) << x.rez << std::fixed << std::setprecision(2) << setw(20) << x.med << "\n";
    }
    out_f2 << output2.str();
    out_f2.close();
    temp.clear();
