        ...
        do
        {
            ...
            if (nd) //tikrinama, ar ivestas skaicius
            {
                if (nd >= 0 && nd <= 10) //tikrinama, ar pazymys yra nuo 0 iki 10
                {
                    ...
                    temp.nd.push_back(nd); //pazymys pridedamas i deque konteineri
    ...
    else //jei vartotojas pasirenka generuoti skaicius (choice==2)
    {
        ...
        do
        {
            ...
            if (skaitl > 0)
            {
                ...
                {
                    nd = dist(mt);
                    temp.nd.push_back(nd); //balas pridedamas i studento deque konteineri
                    temp.sum += nd;
                }
                cout << "Sugeneruoti pazymiai: ";
                for (const auto& st : temp.nd)
                {
                    cout << st << " ";
                }
                cout << endl;
...
void isvestis_file(deque <Studentas>& temp)
{
    ...
    {
        output << left << setw(20) << x.vardas << left << setw(20) << x.pavarde << std::fixed << std::setprecision(2) << setw(17) << x.rez << std::fixed << std::setprecision(2) << setw(20) << x.med << "\n";
    }
    out_f << output.str();
    out_f.close();
...
void mediana(Studentas& temp)
{
    ...
    if (temp.nd.size() >= 1)
    {
        sort(temp.nd.begin(), temp.nd.end()); //isrusiuojamas deque konteineris didejimo tvarka
...
void read_file(deque <Studentas>& sar, string a)
{
    Studentas st;
    string line;
    int nd, egz, lines;
    ...
    cout << "Failo is " << lines << " eiluciu nuskaitymas ir sudejimas i deque uztruko : " << diff.count() << " s\n";
}

void isvestis_2files(deque <Studentas>& temp, string a, string b)
{
    stringstream output;
    deque <Studentas> bad;
    int dydis = temp.size();
    ...
    {
        output << left << setw(20) << bad.at(i).vardas << left << setw(20) << bad.at(i).pavarde << std::fixed << std::setprecision(2) << setw(17) << bad.at(i).rez << std::fixed << std::setprecision(2) << setw(20) << bad.at(i).med << "\n";
    }
    out_f << output.str();
    out_f.close();
    bad.clear();
    ...
    {
        output2 << left << setw(20) << temp.at(i).vardas << left << setw(20) << temp.at(i).pavarde << std::fixed << std::setprecision(2) << setw(17) << temp.at(i).rez << std::fixed << std::setprecision(2) << setw(20) << temp.at(i).med << "\n";
    }
    out_f2 << output2.str();
    out_f2.close();
    temp.clear();
