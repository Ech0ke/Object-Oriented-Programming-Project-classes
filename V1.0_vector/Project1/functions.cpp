...
void isvestis_file(vector <Studentas>& temp)
{
    ...
    {
        output << left << setw(20) << temp.at(i).vardas << left << setw(20) << temp.at(i).pavarde << std::fixed << std::setprecision(2) << setw(17) << temp.at(i).rez << std::fixed << std::setprecision(2) << setw(20) << temp.at(i).med << "\n";
    }
    out_f << output.str();
    out_f.close();
