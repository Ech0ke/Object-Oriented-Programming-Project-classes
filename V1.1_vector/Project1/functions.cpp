...
void Studentas::mediana(Studentas & temp)
{
    double med;
    if (temp.nd_.size() >= 1)
    {
        sort(temp.nd_.begin(), temp.nd_.end()); //isrusiuojamas vector konteineris didejimo tvarka
        if (temp.nd_.size() % 2 == 0) //jei yra lyginis skaicius elementy
        {
            med = (temp.nd_[temp.nd_.size() / 2 - 1] + temp.nd_[temp.nd_.size() / 2]) / 2; //sudedami 2 viduriniai elementai ir dalinama is 2
        }
        else //jei nelyginis skaicius elementu, paimamas vidurinis elementas
        {
            med = temp.nd_[temp.nd_.size() / 2];
        }
    }
    else
    {
    ...
    temp.med_ = 0.4 * med + 0.6 * temp.egz_; //skaiciuojamas galutinis balas
}
...
void zmogus::gen_map(zmogus& temp)
{
...
void zmogus::naujas_failas(zmogus zm, int nd, int sk)
{
    ...
    {
        gen_map(zm);
        output << left << setw(20) << zm.vardas << setw(20) << zm.pavarde;
        for (int j = 0; j < nd; j++)
        {
            pazym = val(mt);
...
void Studentas::read_file(vector <Studentas>& sar, string a)
{
    Studentas st;
    string line;
    int nd, egz, lines;
    ...
    {
        my_buffer >> st.vardas_;
        my_buffer >> st.pavarde_;
...
void Studentas::isvestis_2files(vector <Studentas>& temp, string a, string b)
{
    stringstream output;
    ...
    {
        output << left << setw(20) << bad.at(i).vardas_ << left << setw(20) << bad.at(i).pavarde_ << std::fixed << std::setprecision(2) << setw(17) << bad.at(i).rez_ << std::fixed << std::setprecision(2) << setw(20) << bad.at(i).med_ << "\n";
    }
    out_f << output.str();
    out_f.close();
    bad.clear();
    ...
    {
        output2 << left << setw(20) << temp.at(i).vardas_ << left << setw(20) << temp.at(i).pavarde_ << std::fixed << std::setprecision(2) << setw(17) << temp.at(i).rez_ << std::fixed << std::setprecision(2) << setw(20) << temp.at(i).med_ << "\n";
    }
    out_f2 << output2.str();
    out_f2.close();
    temp.clear();
